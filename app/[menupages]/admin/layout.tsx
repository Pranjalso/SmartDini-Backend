...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  // Fetch stats and check subscription
  const fetchStats = useCallback(async () => {
    if (!menupages) return;
    try {
      const [statsRes, statusRes] = await Promise.all([
        fetch(`/api/orders/${menupages}?status=pending`),
        fetch(`/api/cafes/${menupages}/public`)
      ]);
      ...
      if (statsData.success && statsData.stats) {
        const newCount = statsData.stats.pending || 0;
        
        setNewOrdersCount(newCount);
        
        // Mark first load as complete after the first check
        isFirstLoad.current = false;
      }

      if (statusData.success && statusData.data) {
        setSubscriptionExpired(!!statusData.data.isExpired);
        setCafeDeactivated(!!statusData.data.isManuallyDeactivated);
      }
    } catch (err) {
  ...
  useEffect(() => {
    ...
    // Listen for custom refresh events from child pages
    window.addEventListener('refresh-admin-stats', fetchStats);
    
    // Poll for stats every 10 seconds (Industry standard for dashboard)
      ...
      <div className="flex h-screen overflow-hidden">
        ...
        <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
          ...
          <header className="bg-[#D92632] px-6 py-4 flex items-center gap-4 shadow-md shrink-0 z-30">
            ...
            <div className="flex items-center gap-3">
              ...
              <button
                ...
                className="w-9 h-9 rounded-full bg-white/10 hover:bg-white/20 flex items-center justify-center transition-colors mr-1"
                title={soundEnabled ? "Mute notifications" : "Unmute notifications"}
              >
                {soundEnabled && audioUnlocked ? (
                  <Volume2 size={18} className="text-white" />
                ) : (
                  <VolumeX size={18} className="text-white opacity-60" />
                )}
              </button>
          ...
          <main className="flex-1 overflow-y-auto p-4 md:p-6 lg:p-8 relative">
            {/* Audio Unlock Banner - Professional Amber Theme, only on New Orders Page */}
            {!audioUnlocked && isNewOrdersPage && (
              <div className="mb-6 bg-amber-50 border border-amber-200 p-4 rounded-2xl flex flex-col sm:flex-row items-center justify-between gap-4 shadow-sm animate-in fade-in slide-in-from-top-4 duration-300">
                <div className="flex items-center gap-4 text-amber-800">
                  <div className="w-10 h-10 rounded-full bg-white flex items-center justify-center shadow-sm">
                    <Volume2 size={20} className="animate-pulse text-amber-600" />
                  </div>
                  <div>
                    <p className="font-bold text-sm sm:text-base">Enable Order Alerts</p>
                    <p className="text-xs sm:text-sm opacity-80 text-amber-700">Please click the button to enable real-time sound notifications for incoming orders.</p>
                  </div>
                </div>
                <button 
                  onClick={handleUnlockAudio}
                  className="w-full sm:w-auto px-6 py-2.5 bg-amber-600 text-white text-sm font-bold rounded-xl hover:bg-amber-700 transition-all shadow-md active:scale-95"
                >
                  Enable Sound
                </button>
              </div>
